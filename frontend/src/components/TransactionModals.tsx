    ...
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          ...
          <select value={formData.accountId} onChange={(e) => setFormData({ ...formData, accountId: parseInt(e.target.value) })} className={IC} required disabled={loading}>
            {accounts.map((a) => <option key={a.id} value={a.id}>{a.accountName}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-2">Type</label>
          <select value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value as 'INCOME' | 'EXPENSE' })} className={IC} required disabled={loading}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div>
          ...
          <input type="datetime-local" value={formData.transactionDate} onChange={(e) => setFormData({ ...formData, transactionDate: e.target.value })} className={IC} required disabled={loading} />
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-2">Category / Tag</label>
          <input type="text" list="category-suggestions" value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className={IC} placeholder="Food, Salary, Rent, etc." disabled={loading} />
        ...
        <div>
          ...
          <input type="text" value={formData.senderReceiver} onChange={(e) => setFormData({ ...formData, senderReceiver: e.target.value })} className={IC} placeholder="Person or company" disabled={loading} />
        </div>
      </div>
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-2">Description</label>
        <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className={`${IC} resize-none`} rows={2} placeholder="Why was this spent?" disabled={loading} />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-2">Payment Method</label>
          <select value={formData.paymentMethod} onChange={(e) => setFormData({ ...formData, paymentMethod: e.target.value as FormData['paymentMethod'] })} className={IC} required disabled={loading}>
        ...
        <div>
          ...
          <input type="text" value={formData.paymentDetails} onChange={(e) => setFormData({ ...formData, paymentDetails: e.target.value })} className={IC} placeholder="UPI ID, Card name, etc." disabled={loading} />
        </div>
      </div>
...
export const AddTransactionModal: React.FC<{ accounts: Account[]; onClose: () => void }> = ({ accounts, onClose }) => {
